...
int LibV1::ISI_values(mapStr2intVec& IntFeatureData,
    ...
    if (intFeatures.at("ignore_first_ISI").size() > 0 && intFeatures.at("ignore_first_ISI")[0] == 0) {
        IgnoreFirstISI = 0;
    }

    vector<double> VecISI;
    for (size_t i = IgnoreFirstISI + 1; i < doubleFeatures.at("peak_time").size(); i++)
        VecISI.push_back(doubleFeatures.at("peak_time")[i] - doubleFeatures.at("peak_time")[i - 1]);
...
int LibV1::firing_rate(mapStr2intVec& IntFeatureData,
                       mapStr2doubleVec& DoubleFeatureData,
                       mapStr2Str& StringData) {
    const auto& doubleFeatures = getFeatures(DoubleFeatureData, {"peak_time", "stim_start", "stim_end"});
    double lastAPTime = 0.;
    int nCount = 0;
    for (const auto& time : doubleFeatures.at("peak_time")) {
        if ((time >= doubleFeatures.at("stim_start")[0]) && (time <= doubleFeatures.at("stim_end")[0])) {
            lastAPTime = time;
            nCount++;
        }
    }
    ...
    firing_rate.push_back(nCount * 1000 / (lastAPTime - doubleFeatures.at("stim_start")[0]));
...
int LibV1::first_spike_time(mapStr2intVec& IntFeatureData,
    ...
    first_spike.push_back(doubleFeatures.at("peak_time")[0] - doubleFeatures.at("stim_start")[0]);
...
int LibV1::AP_height(mapStr2intVec& IntFeatureData,
    ...
    setVec(DoubleFeatureData, StringData, "AP_height", doubleFeatures.at("peak_voltage"));
...
int LibV1::AP_amplitude(mapStr2intVec& IntFeatureData,
                        mapStr2doubleVec& DoubleFeatureData,
                        mapStr2Str& StringData) {
    const auto& doubleFeatures = getFeatures(DoubleFeatureData, {"V", "stim_start", "stim_end", "peak_voltage", "peak_time"});
    ...
    if (doubleFeatures.at("peak_voltage").size() != doubleFeatures.at("peak_time").size()) {
        GErrorStr += "AP_amplitude: Not the same amount of peak_time and peak_voltage entries";
        return -1;
    }
    vector<double> peakvoltage_duringstim;
    for (size_t i = 0; i < doubleFeatures.at("peak_time").size(); i++) {
        if (doubleFeatures.at("peak_time")[i] >= doubleFeatures.at("stim_start")[0] && doubleFeatures.at("peak_time")[i] <= doubleFeatures.at("stim_end")[0]) {
    ...
    if (peakvoltage_duringstim.size() > intFeatures.at("AP_begin_indices").size()) {
        GErrorStr += "AP_amplitude: More peak_voltage entries during the stimulus than AP_begin_indices entries";
        return -1;
    }
    vector<double> apamplitude;
    apamplitude.resize(peakvoltage_duringstim.size());
    for (size_t i = 0; i < apamplitude.size(); i++) {
        apamplitude[i] = peakvoltage_duringstim[i] - doubleFeatures.at("V")[intFeatures.at("AP_begin_indices")[i]];
    }
...
int LibV1::AHP_depth_abs_slow(mapStr2intVec& IntFeatureData,
                              mapStr2doubleVec& DoubleFeatureData,
                              mapStr2Str& StringData) {
    const auto& doubleFeatures = getFeatures(DoubleFeatureData, {"T", "V", "sahp_start"});
    const auto& intFeatures = getFeatures(IntFeatureData, {"peak_indices"});
    if (intFeatures.at("peak_indices").size() < 3) {
        GErrorStr += "\n At least 3 spikes needed for AHP_depth_abs_slow and AHP_slow_time.\n";
        return -1;
    }
    double sahp_start = (doubleFeatures.at("sahp_start").empty()) ? 5 : doubleFeatures.at("sahp_start")[0];
    vector<int> adas_indices;
    ...
    for (size_t i = 0; i < adas_indices.size(); i++) {
        ...
        ahpslowtime[i] = (doubleFeatures.at("T")[adas_indices[i]] - doubleFeatures.at("T")[intFeatures.at("peak_indices")[i + 1]]) /
                         (doubleFeatures.at("T")[intFeatures.at("peak_indices")[i + 2]] - doubleFeatures.at("T")[intFeatures.at("peak_indices")[i + 1]]);
    }
    if (retval >= 0) {
        setVec(DoubleFeatureData, StringData, "AHP_depth_abs_slow", ahpdepthabsslow);
...
int LibV1::burst_ISI_indices(mapStr2intVec& IntFeatureData,
                             mapStr2doubleVec& DoubleFeatureData,
                             mapStr2Str& StringData) {
    const auto& doubleFeatures = getFeatures(DoubleFeatureData, {"ISI_values", "burst_factor"});
    const auto& intFeatures = getFeatures(IntFeatureData, {"peak_indices"});
    if (intFeatures.at("peak_indices").size() < 5) {
        GErrorStr += "\nError: More than 5 spike is needed for burst calculation.\n";
        return -1;
    }
    double BurstFactor = (doubleFeatures.at("burst_factor").empty()) ? 2 : doubleFeatures.at("burst_factor")[0];
    vector<int> BurstIndex;
    int retVal = __burst_ISI_indices(
        BurstFactor,
        intFeatures.at("peak_indices"),
        doubleFeatures.at("ISI_values"),
        BurstIndex
    );
    if (retVal >= 0) {
...
static int __burst_mean_freq(const vector<double>& PVTime, const vector<int>& BurstIndex,
                             int IgnoreFirstISI,
...
int LibV1::burst_mean_freq(mapStr2intVec& IntFeatureData,
                           ...
                           mapStr2Str& StringData) {
    const auto& doubleFeatures = getFeatures(DoubleFeatureData, {"peak_time"});
    const auto& intFeatures = getFeatures(IntFeatureData, {"burst_ISI_indices", "ignore_first_ISI"});
    int IgnoreFirstISI = 1;
    int retValIgnore;
...
int LibV1::burst_number(mapStr2intVec& IntFeatureData,
                        mapStr2doubleVec& DoubleFeatureData,
                        mapStr2Str& StringData) {
    const auto& doubleFeatures = getFeatures(DoubleFeatureData, {"burst_mean_freq"});
    vector<int> BurstNum;
...
static int __interburst_voltage(const vector<int>& BurstIndex, const vector<int>& PeakIndex,
...
int LibV1::interburst_voltage(mapStr2intVec& IntFeatureData,
    ...
    const auto& intFeatures = getFeatures(IntFeatureData, {"peak_indices", "burst_ISI_indices"});
    int IgnoreFirstISI;
    int retValIgnore;
    vector<int> retIgnore;
    retValIgnore = getParam(IntFeatureData, "ignore_first_ISI", retIgnore);
...
int LibV1::adaptation_index(mapStr2intVec& IntFeatureData,
                            mapStr2doubleVec& DoubleFeatureData,
                            mapStr2Str& StringData) {
    const auto& doubleFeatures = getFeatures(DoubleFeatureData, {"peak_time", "stim_start", "stim_end", "spike_skipf"});
    const auto& intFeatures = getFeatures(IntFeatureData, {"max_spike_skip"});

    if (doubleFeatures.at("spike_skipf")[0] < 0 || doubleFeatures.at("spike_skipf")[0] >= 1) {
    ...
    int retVal = __adaptation_index(
        doubleFeatures.at("spike_skipf")[0],
        intFeatures.at("max_spike_skip")[0],
...
int LibV1::adaptation_index2(mapStr2intVec& IntFeatureData,
    ...
    if (retval >= 0) {
        setVec(DoubleFeatureData, StringData, "adaptation_index2", adaptationindex2);
    }
    return retval;
}
...
int LibV1::trace_check(mapStr2intVec& IntFeatureData,
    ...
    for (const auto& peak : doubleFeatures.at("peak_time")) {
        if (peak < doubleFeatures.at("stim_start")[0] || peak > doubleFeatures.at("stim_end")[0] * 1.05) {
            sane = false;
            break;
        }
    }
    if (sane) {
        ...
        GErrorStr += "Trace sanity check failed, there were spike outside the stimulus interval.\n";
        return -1;
    }
}
...
int LibV1::ohmic_input_resistance(mapStr2intVec& IntFeatureData,
                                  mapStr2doubleVec& DoubleFeatureData,
                                  mapStr2Str& StringData) {
    const auto& doubleFeatures = getFeatures(DoubleFeatureData, {"voltage_deflection", "stimulus_current"});
    vector<double> oir;
    int retVal = __ohmic_input_resistance(
        doubleFeatures.at("voltage_deflection")[0],
...
int LibV1::steady_state_voltage(mapStr2intVec& IntFeatureData,
                                mapStr2doubleVec& DoubleFeatureData,
                                mapStr2Str& StringData) {
    const auto& doubleFeatures = getFeatures(DoubleFeatureData, {"V", "T", "stim_end"});
    if (doubleFeatures.at("stim_end").size() != 1) return -1;

    vector<double> ssv;
    int retVal = __steady_state_voltage(
        doubleFeatures.at("V"),
        doubleFeatures.at("T"),
        doubleFeatures.at("stim_end")[0],
        ssv
...
int LibV1::single_burst_ratio(mapStr2intVec& IntFeatureData,
    ...
    const auto& doubleFeatures = getFeatures(DoubleFeatureData, {"ISI_values"});
    vector<double> singleburstratio;
    int retval = __single_burst_ratio(
        doubleFeatures.at("ISI_values"),
        singleburstratio
    );
    if (retval > 0) {
        setVec(DoubleFeatureData, StringData, "single_burst_ratio", singleburstratio);
    }
    return retval;
}
...
int LibV1::AP_width(mapStr2intVec& IntFeatureData,
                    mapStr2doubleVec& DoubleFeatureData,
                    mapStr2Str& StringData) {
    const auto& doubleFeatures = getFeatures(DoubleFeatureData, {"T", "V", "Threshold", "stim_start", "stim_end"});
    const auto& intFeatures = getFeatures(IntFeatureData, {"peak_indices", "min_AHP_indices", "strict_stiminterval"});
    bool strict_stiminterval = intFeatures.at("strict_stiminterval").size() > 0 ? bool(intFeatures.at("strict_stiminterval")[0]) : false;
    vector<double> apwidth;
...
int LibV1::doublet_ISI(mapStr2intVec& IntFeatureData,
    ...
    vector<double> doubletisi(1, doubleFeatures.at("peak_time")[1] - doubleFeatures.at("peak_time")[0]);
...
int LibV1::AHP_depth_slow(mapStr2intVec& IntFeatureData,
                     mapStr2doubleVec& DoubleFeatureData,
                     mapStr2Str& StringData) {
    const auto& doubleFeatures = getFeatures(DoubleFeatureData, {"voltage_base", "AHP_depth_abs_slow"});
    vector<double> ahpdepthslow;
    int retval = __AHP_depth_slow(
        doubleFeatures.at("voltage_base"),
        doubleFeatures.at("AHP_depth_abs_slow"),
        ahpdepthslow
    );
    if (retval > 0) {
...
int LibV1::AHP_depth(mapStr2intVec& IntFeatureData,
                     mapStr2doubleVec& DoubleFeatureData,
                     mapStr2Str& StringData) {
    const auto& doubleFeatures = getFeatures(DoubleFeatureData, {"voltage_base", "min_AHP_values"});
    vector<double> ahpdepth;
    int retval = __AHP_depth(
        doubleFeatures.at("voltage_base"),
        doubleFeatures.at("min_AHP_values"),
        ahpdepth
    );
    if (retval > 0) {
...
int LibV1::AP_amplitude_diff(mapStr2intVec& IntFeatureData,
    ...
    vector<double> apamplitudediff;
    int retval = __AP_amplitude_diff(
...
int LibV1::AHP_depth_diff(mapStr2intVec& IntFeatureData,
    ...
    vector<double> ahpdepthdiff;
    int retval = __AHP_depth_diff(
        doubleFeatures.at("AHP_depth"),
        ahpdepthdiff
    );
    if (retval > 0) {

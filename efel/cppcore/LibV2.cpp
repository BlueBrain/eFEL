...
int LibV2::__AP_duration_half_width(const vector<double>& t,
  ...
  for (int i = 0; i < apdurationhalfwidth.size(); i++) {
...
int LibV2::__AP_rise_time(const vector<double>& t,
  ...
  for (int i = 0; i < aprisetime.size(); i++) {
...
int LibV2::__AP_fall_time(const vector<double>& t,
  ...
  for (int i = 0; i < apfalltime.size(); i++) {
...
int LibV2::__AP_rise_rate(const vector<double>& t, const vector<double>& v,
  ...
  for (int i = 0; i < apriserate.size(); i++) {
...
int LibV2::__AP_fall_rate(const vector<double>& t, const vector<double>& v,
  ...
  for (int i = 0; i < apfallrate.size(); i++) {
...
int LibV2::__AP_amplitude_change(const vector<double>& apamplitude,
  ...
  for (int i = 0; i < apamplitudechange.size(); i++) {
    apamplitudechange[i] =
...
int LibV2::__AP_duration_change(const vector<double>& apduration,
  ...
  for (int i = 0; i < apdurationchange.size(); i++) {
...
int LibV2::__AP_duration_half_width_change(
  ...
  for (int i = 0; i < apdurationhalfwidthchange.size(); i++) {
...
int LibV2::__AP_rise_rate_change(const vector<double>& apriserate,
  ...
  for (int i = 0; i < apriseratechange.size(); i++) {
...
int LibV2::__AP_fall_rate_change(const vector<double>& apfallrate,
  ...
  for (int i = 0; i < apfallratechange.size(); i++) {
...
int LibV2::__fast_AHP_change(const vector<double>& fastahp,
  ...
  for (int i = 0; i < fastahpchange.size(); i++) {
...
int LibV2::__max_amp_difference(const vector<double>& peakvoltage,
  ...
  for (int i = 0; i < diff_peak_voltage.size(); i++) {
...
// *** E29 ***
int LibV2::E29(mapStr2intVec& IntFeatureData,
               mapStr2doubleVec& DoubleFeatureData, mapStr2Str& StringData) {
  ...
  int nsize;
  retval =
      CheckInDoublemap(DoubleFeatureData, StringData, string("E29"), nsize);
  if (retval) {
    return nsize;
  }
  vector<string> stim_params;
  // getTraces(DoubleFeatureData, "IV", stim_params);
  if (stim_params.size() > 1) {
    vector<double> current(stim_params.size());
    vector<double> steady_state_hyper(stim_params.size());
    for (int i = 0; i < stim_params.size(); i++) {
      vector<double> stimulus_current;
      getDoubleParam(DoubleFeatureData, "stimulus_current" + stim_params[i],
                     stimulus_current);
      current[i] = stimulus_current[0];
      vector<double> steady;
      getDoubleParam(DoubleFeatureData, "steady_state_hyper" + stim_params[i],
                     steady);
      steady_state_hyper[i] = steady[0];
    }
    vector<double> slope;
    slope_straight_line_fit(current, steady_state_hyper, slope);
    vector<double> e29(1, slope[0]);
    vector<double> e29_cod(1, slope[2]);
    setDoubleVec(DoubleFeatureData, StringData, "E29", e29);
    setDoubleVec(DoubleFeatureData, StringData, "E29_cod", e29_cod);
    return e29.size();
  }
  GErrorStr += "\nMore than 1 trace required for calculation of E29";
  return -1;
}

// standard deviation of the ohmic_input_resistance over repetitions
...
// time constant, regular feature
int LibV2::time_constant_mean(mapStr2intVec& IntFeatureData,
                              mapStr2doubleVec& DoubleFeatureData,
                              mapStr2Str& StringData) {
  // These features are disabled, since there is a serious bug in getTraces()
  GErrorStr += "\ntime_constant_mean not implemented\n";
  return -1;

  int retval;
  int nsize;
  retval = CheckInDoublemap(DoubleFeatureData, StringData, "time_constant_mean",
                            nsize);
  if (retval) {
    return nsize;
  }
  vector<double> time_constant_mean;
  // retval = mean_traces_double(DoubleFeatureData, "time_constant", "IV", 0,
  // time_constant_mean);
  retval = -1;
  if (retval >= 0) {
    setDoubleVec(DoubleFeatureData, StringData, "time_constant_mean",
                 time_constant_mean);
  }
  return retval;
}

// standard deviation of the time constant over repetitions
int LibV2::time_constant_std(mapStr2intVec& IntFeatureData,
                             mapStr2doubleVec& DoubleFeatureData,
                             mapStr2Str& StringData) {
  // These features are disabled, since there is a serious bug in getTraces()
  GErrorStr += "\ntime_constant_std not implemented\n";
  return -1;

  int retval;
  int nsize;
  retval = CheckInDoublemap(DoubleFeatureData, StringData, "time_constant_std",
                            nsize);
  if (retval) {
    return nsize;
  }
  vector<double> time_constant_mean;
  retval = getDoubleVec(DoubleFeatureData, StringData, "time_constant_mean",
                        time_constant_mean);
  if (retval < 0) return -1;
  vector<double> time_constant_std;
  // retval = std_traces_double(DoubleFeatureData, "time_constant", "IV",
  // time_constant_mean[0], 0, time_constant_std);
  retval = -1;
  if (retval >= 0) {
    setDoubleVec(DoubleFeatureData, StringData, "time_constant_std",
                 time_constant_std);
  }
  return retval;
}
// end of time_constant, regular feature

// ohmic input resistance, regular feature
int LibV2::ohmic_input_resistance_mean(mapStr2intVec& IntFeatureData,
                                       mapStr2doubleVec& DoubleFeatureData,
                                       mapStr2Str& StringData) {
  // These features are disabled, since there is a serious bug in getTraces()
  GErrorStr += "\nohmic_input_resistance_mean not implemented\n";
  return -1;

  int retval;
  int nsize;
  retval = CheckInDoublemap(DoubleFeatureData, StringData,
                            "ohmic_input_resistance_mean", nsize);
  if (retval) {
    return nsize;
  }
  vector<double> ohmic_input_resistance_mean;
  vector<double> ohmic_input_resistance_std;
  // retval = mean_traces_double(DoubleFeatureData, "ohmic_input_resistance",
  // "IV", 0, ohmic_input_resistance_mean);
  retval = -1;
  if (retval >= 0) {
    retval = std_traces_double(DoubleFeatureData, "ohmic_input_resistance",
                               "IV", ohmic_input_resistance_mean[0], 0,
                               ohmic_input_resistance_std);
    setDoubleVec(DoubleFeatureData, StringData, "ohmic_input_resistance_mean",
                 ohmic_input_resistance_mean);
    setDoubleVec(DoubleFeatureData, StringData, "ohmic_input_resistance_std",
                 ohmic_input_resistance_std);
  }
  return retval;
}
...
// standard deviation of the ohmic_input_resistance over repetitions
int LibV2::ohmic_input_resistance_std(mapStr2intVec& IntFeatureData,
                                      mapStr2doubleVec& DoubleFeatureData,
                                      mapStr2Str& StringData) {
  // These features are disabled, since there is a serious bug in getTraces()
  GErrorStr += "\nohmic_input_resistance_std not implemented\n";
  return -1;

  int retval;
  int nsize;
  retval = CheckInDoublemap(DoubleFeatureData, StringData,
                            "ohmic_input_resistance_std", nsize);
  return retval;
}
// end of time_constant, regular feature
//
